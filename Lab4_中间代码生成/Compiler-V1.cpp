...
int PrimaryExp(bool bk, string &ans)
{
    ...
    else
    {
        ...
        pos++;
        int ret = 0;
        bool index_is_zero = false; // Flag to track if index is known to be 0 at runtime
        if (code[pos].name == "[")
        {
            pos++;
            if (bk)
            {
                ret = AddExp(1, ans); // Compile-time index calculation
            }
            else
            {
                // 优化：常量下标直接用立即数左移
        ...
        else
        {
            ...
            {
                ans += "	li $t1, 0\n"; // Runtime offset is 0
                index_is_zero = true;     // Mark index as zero for runtime optimization
            }
            // If bk is true, ret remains 0, which is correct for base address access.
        }
        if (!bk) // Runtime value calculation
        {
            statu stu = st.find(now);
            if (stu.bk)
            {
                ans += "	la $t2, " + now + "\n"; // Load base address (global)
            }
            else
            {
                ans += "	la $t2, " + my_to_string(stu.offset) + "($sp)\n"; // Load base address (local)
            }

            // Optimization: if index was 0, replace add with move
            if (index_is_zero)
            {
                ans += "	move $t1, $t2\n"; // Final address = base + 0
            }
            else
            {
            ...
            ans += "	lw $t1, ($t1)\n"; // Load value from final address
            ans += "	lw $t0, ($sp)\n";
            ans += "	add $t0, $t0, 4\n";
            ans += "	sw $t0, ($sp)\n";
            ans += "	sw $t1, ($t0)\n"; // Store loaded value onto stack
        }
        else // Compile-time value calculation
        {
            // Ensure 'ret' (compile-time index) is valid before accessing val vector
            ...
            {
                // Handle potential out-of-bounds access during compile-time evaluation if necessary.
                // Assuming valid index based on previous runs.
                // If the source language guarantees bounds checking or validity, this is okay.
                // For robustness, an error or default might be better.
                ret = st.mp[now].back().val[ret]; // Original line
            }
        }
        return ret;
    }
